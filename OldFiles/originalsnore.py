...
action = OptionMenu(root, 
                    action_var, 
                    *action_options)
action.grid(column=1,row=2)

label_var=StringVar()
label = Label(root, width=8,
...
direction.grid(column=5,row=2)

#InputBlock_DESTINATION_IPAddress
destination_ip = Entry(root,width=10,bg='light blue')
destination_ip.grid(column=6,row=2)

#InputBlock_DESTINATION_Port
destination_port = Entry(root,width=5,bg='sea green')
destination_port.grid(column=7,row=2)

#InputBlock_LastField
lastfield = Entry(root,width=40,bg='slate gray')
lastfield.grid(column=8,row=2)

def enterRule():
    text = action_var.get() + " " + proto_var.get() + " " 
    + source_ip.get() + " " + source_port.get() + " " 
    + direction_var.get() + " " + destination_ip.get() 
    + " " + destination_port.get() + " " + lastfield.get()
